...
def jepa_ramp_at(step: int, jepa_warmup: int, ramp_steps: int = 1000) -> float:
    """Hold ramp at 0 for the first jepa_warmup steps, then linear ramp to 1.

    Used to gate both λ_jepa and λ_conv_jepa: the LM head needs to lock in
    basic byte fluency before any latent-target loss kicks in, otherwise
    the head and the residual fight each other and the model never escapes
    uniform-noise output.
    """
    if step < jepa_warmup:
        return 0.0
    return min(1.0, (step - jepa_warmup) / ramp_steps)


def jepa_weight_at(step: int, target: float, jepa_warmup: int,
                   ramp_steps: int = 1000) -> float:
    """Back-compat wrapper for callers that want target * ramp directly."""
    return target * jepa_ramp_at(step, jepa_warmup, ramp_steps)
...
# ---------------------------------------------------------------------------
class MixedIterator:
    """Round-by-RNG sampler from teacher / biling / unary / conv iterators."""
    ...
    def __init__(self, cfg: TrainConfig, rng: np.random.Generator):
        ...
        self._unary = CountingByteIterator(cfg.batch_size,
                                           seed=int(rng.integers(0, 2**31)))
        self._kinds = ["teacher", "biling", "unary"]
        self._its = {"teacher": self._teacher, "biling": self._biling,
                     "unary": self._unary}
        weights = [cfg.mix_teacher, cfg.mix_biling, cfg.mix_unary]
        if cfg.subtitle_path and cfg.mix_conv > 0:
            sub_ds = TeacherThoughtsDataset(cfg.subtitle_path)
            self._conv = TeacherIterator(sub_ds, cfg.batch_size,
                                         max_bytes=cfg.seq_len,
                                         seed=int(rng.integers(0, 2**31)))
    ...
    def __next__(self) -> tuple[str, Batch]:
        kind = self.rng.choice(self._kinds, p=self._weights)
        return kind, next(self._its[kind])
...
def train(cfg: TrainConfig) -> None:
    ...
    pusher = None
    if _HAS_PUSHER:
        kind = Path(__file__).resolve().parent.name        # "jepa"
    ...
    for step in range(start_step, cfg.steps):
        ...
        for pg in opt.param_groups:
            pg["lr"] = lr
        ramp = jepa_ramp_at(step, cfg.jepa_warmup)
        jw = cfg.lambda_jepa * ramp
        cw = cfg.lambda_conv_jepa * ramp
        ...
        with torch.amp.autocast(device_type=device.type, dtype=torch.bfloat16):
            if kind == "teacher":
                ...
                # student residual. Same warmup gate as l_jepa — λ ramps from 0
                # so the LM head can lock in basic fluency first.
                plens = batch.prompt_lens.to(device)
                logits, prim_out, residual, intent = model(
                    tokens, return_jepa=True, prompt_lens=plens,
                )
        ...
        if step % 50 == 0:
            ...
            if pusher is not None:
                ...
                pusher.metrics(step=step, **{
                    ...
                    ("loss", "l_byte", "l_jepa", "l_conv", "l_sig", "l_aux",
                     "lr", "jepa_w")
                })
...
def main():
    ...
    ap.add_argument("--mix-conv", type=float, default=TrainConfig.mix_conv)
    ...
    ap.add_argument("--subtitle-path", default=TrainConfig.subtitle_path,
                    help="JEPT-format subtitle thoughts (response-end hidden); "
                         "enables conv-jepa when --mix-conv > 0.")
