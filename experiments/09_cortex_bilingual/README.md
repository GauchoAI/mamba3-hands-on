...
- `train_bilingual_cortex_lm.py` — PyTorch trainer for a `CortexLM`
  ...
  mixin built by `lab_platform.make_bilingual_corpus`).
...
- `parity_mlx.py` — numerical parity test between the PyTorch
  reference (`lab_platform.cortex_counting`) and the MLX port.
...
PYTHONPATH=src python -m lab_platform.make_bilingual_corpus        # -> data/bilingual.txt
...
# -> repo root, so `from lab_platform.cortex_counting import ...` works
```
...
The shared cortex foundation (`lab_platform.cortex_counting`) lives in
`src/lab_platform/` and is *not* duplicated here. That contrasts with the `jepa/`
