---
title: Operator-guided decoding
chapter: "14"
status: active
lab_book: subsection
summary: "Put a learned validity operator inside a noisy generation loop."
---

# Chapter 14 — Operator-Guided Decoding

Compare a noisy role generator with the same generator constrained at each step
by a learned stack-validity operator.

```bash
.venv/bin/python experiments/14_operator_guided_decoding/operator_guided_decoding.py
```
