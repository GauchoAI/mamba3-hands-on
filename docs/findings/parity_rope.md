# Parity / RoPE / minimal-block foundational arc — findings journal

The early architecture experiments: setting up the minimal SISO block,
discovering the operator-precedence bug, finding mechanistically that
the model learned the parity rotation at exactly –π, ablating RoPE,
mod-3 modular counting, and selective-copy via gating.

This is the project's origin arc. Everything that came after — the
counter primitive, the EOS-bias gate, LoopCounter, JEPA-Cortex — owes
its existence to the lessons here. Entries 1–7 of the original
findings.md.
