...
| 12 | [`experiments/12_operator_curriculum_intro/`](experiments/12_operator_curriculum_intro/) | active scaffolding | Consolidates the prior one-minute operator scripts as subsections |
| 13 | [`experiments/13_latent_operator_discovery/`](experiments/13_latent_operator_discovery/) | **active** | GRU prefix encoder discovers stack-relevant latent state from raw traces |

Each chapter has its own `README.md` with synopsis, status, and a
cross-link to the relevant `docs/findings/<topic>.md` entry.

The Lab Book chapter manifest is generated by
[`tools/generate_lab_book_manifest.py`](tools/generate_lab_book_manifest.py).
Use `lab_book: subsection` in experiment front matter when a runnable script is
supporting evidence rather than a top-level chapter.
