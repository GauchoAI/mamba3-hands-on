...
class CortexLM(nn.Module):
    ...
    def forward(self, tokens: torch.Tensor, return_aux: bool = False,
                return_jepa: bool = False, prompt_lens: torch.Tensor | None = None):
        """
        tokens: (B, L) long
        return_aux=False, return_jepa=False -> logits (B, L, vocab)
        return_aux=True  -> (logits, prim_outputs: dict[name -> dict])
        return_jepa=True -> (logits, prim_outputs, residual, intent)
            residual: (B, L, d_model) — post-final-norm hidden, what the
                      ThoughtHead reads
            intent:   (B, d_model) — mean-pool of residual over prompt
                      positions; if prompt_lens is None falls back to mean
        ...
        if return_jepa:
            ...
            if prompt_lens is None:
                ...
                m = (arange < prompt_lens.unsqueeze(1)).float().unsqueeze(-1)
                denom = m.sum(dim=1).clamp_min(1.0)
                intent = (x * m).sum(dim=1) / denom
            return logits, prim_outputs, x, intent
        if return_aux:
