# `engine/ptx/` — PTX Engine Findings Journal

Hand-written PTX Mamba-3 engine for CUDA — the H100/vast.ai-era work
that owns the entire forward+backward pass without PyTorch's CUDA
runtime. This file holds the entries chronologically; entries 27–54
were originally in the root `findings.md` and were moved here as part
of the project's structuring pass (the root file had grown into a
single multi-thousand-line document spanning many subprojects).

The PTX arc spans 2026-04-24 to 2026-04-26. By the end of it:
- Forward bit-parity vs CPU and PyTorch
- Backward parity to 1e-6 across the full chain
- Training convergence matches PyTorch on the same stream, ~14× faster
- ptxd daemon mode with hot-plug job submission
- Streaming batch protocol (BTCH format)
- AdamW optimizer state round-trip, checkpoint compatibility with
  the existing 82 .pt files
- GA mutation surface: real Lion + Focal + LabelSmooth kernels (no
  more silent fallbacks)

Contemporary status (2026-04-26 onward, per the root findings.md):
the PTX/H100 era moved to the `pod-archive` branch. The Mac-only
daily-driver path went back to PyTorch + MPS via
`specialist_trainer.py`, and the production GA orchestrator is
`three_populations.py`. PTX remains a working artifact and a target
for future big-batch overnight runs once a CUDA box is rented again.
