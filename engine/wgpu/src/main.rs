...
fn run_parity_training() {
    use mamba3_engine::train::TrainState;

    println!("=== Parity Training from Scratch (Pure Rust) ===");

    // Create model from scratch — d=32, L=1, tiny
    let model = Mamba3Model::new_random(32, 16, 16, 1, 260);
    println!("Model: {} params (d=32, L=1)", model.param_count());

    let mut state = TrainState::new(model, 1e-3, 0.1);

    // Simple LCG for data generation
    ...
    for step in 0..total_steps {
        // Accumulate gradients over a mini-batch of 16 examples
        let n_bits = 4; // start easy
        ...
        for _ in 0..batch {
            ...
            total_loss += state.train_step(&tokens, &targets);
        }
        ...
        if (step + 1) % 200 == 0 {
            ...
            for _ in 0..200 {
                ...
                let preds = state.model.predict(&test_tokens);
                // Check prediction at SEP position (last position)
                let pred_answer = *preds.last().unwrap_or(&0);
